...
function GetUserList($orderby = array('type' => 'id' , 'desc' => false )){
	...
	$y=date("Y");
	$insertSQL = sprintf("SELECT user.id,user.name'name',email,phone,depart_id,timediff(endtime,starttime)'timediff',user.depart'departname' FROM user LEFT JOIN depart ON depart.id = 	depart_id WHERE year='$y' " .$orderbySQL);
	//print_r($insertSQL);
...
function GetUserData($id){
	...
	$insertSQL = sprintf("SELECT user.name'name',sId,ident.name'ident',college.name'college',user.depart'depart',email,phone,ipstamp" .
						 ",starttime,endtime,timediff(endtime,starttime)'timediff' FROM user " .
